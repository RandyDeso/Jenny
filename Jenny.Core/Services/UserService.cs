...
/// </summary>
public sealed class UserService(IUserRepository userRepository, IUserStateLock userStateLock) : IUserService
{
    /// <inheritdoc />
    ...
    {
        return await userStateLock.ExecuteAsync(userId, async () =>
        {
            var user = await GetOrCreateInternalAsync(userId, null, cancellationToken);
    ...
    public async Task<IReadOnlyCollection<UserFavorite>> GetFavoritesAsync(Guid userId, CancellationToken cancellationToken = default)
    {
        ...
        {
            var user = await GetOrCreateInternalAsync(userId, null, cancellationToken);
            return (IReadOnlyCollection<UserFavorite>)user.Favorites.OrderByDescending(favorite => favorite.CreatedAt).ToArray();
        });
    }
    ...
    {
        return await userStateLock.ExecuteAsync(userId, () => GetOrCreateInternalAsync(userId, email, cancellationToken));
    }

    private async Task<User> GetOrCreateInternalAsync(Guid userId, string? email, CancellationToken cancellationToken)
    {
